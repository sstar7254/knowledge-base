...
- **소화(output)는 선택이다.** `/inbox`·`/inbox-deep` 마무리 때 유도하거나 나중에 `/digest`로 진행한다.
  소화 안 된 노트를 백로그로 목록화하거나 독촉하지 않는다(망각 = 2차 필터, 원 설계 철학).
  `## 소화`의 흐름은 ① 사람이 자기 언어로 요약 → ② AI가 심화 질문 2~3개 → ③ 답은 선택 →
  ④ AI가 리서치 대비 누락·오해를 피드백 callout으로 기록. 자세한 절차는 `/digest` 스킬이 규정한다.
...
- 캡쳐 흐름: 사람은 `inbox/`(형식 자유)나 채팅에 질문 메모를 던진다. `/inbox`(빠른) 또는
  ...
  이후 소화(output)는 `/digest` 스킬이 담당한다(요약·심화질문·피드백).
