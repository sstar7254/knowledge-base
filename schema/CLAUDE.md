...
5. **종합(moc) 페이지는 `wiki/moc/`에 둔다.** 파일명은 **영문 kebab-case**로 짓는다
   (예: `logistics-tech-startup-map.md`). 한 주제에 대해 **인덱스(목록·링크)와 종합(지형도·분석)을
   별도 파일로 쪼개지 말고 하나의 moc 페이지로 병합**한다(별도 README/개념노트 남발 금지). 상단에 최소
   frontmatter를 둔다:
   ```yaml
   ---
