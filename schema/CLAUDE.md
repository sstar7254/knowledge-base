...
4. **모든 사실 진술은 raw로 거슬러 올라갈 수 있어야 한다.** 출처가 되는 raw 노트를 본문에서 위키링크로 연결한다.
   위키링크는 파일명 기준으로 해소되므로 폴더 위치와 무관하게 작동한다.
5. **종합(moc) 페이지**는 `wiki/` 루트에 두고 `wiki/wiki-map.md` 허브에서 링크한다.
...
## 4. 작업 흐름 (`/research` 스킬이 호출됐을 때)

리서치 요청을 받으면 아래 8단계를 **순서대로 빠짐없이** 따른다. 이 절차가 정본이며, `skills/research/SKILL.md`는 이를 리서치 방법론과 함께 구체화한다.
...
7. **wiki-map 연결** — 작성한 위키를 허브 파일 `wiki/wiki-map.md`에 도메인별로 링크한다.
8. **인덱스 갱신** — 새로 생기거나 이동한 파일·디렉토리 path를 이 `schema/CLAUDE.md`와 `README.md`의 파일 트리에 모두 반영한다.
