...
- `## 리서치`는 raw의 source-digest 원칙을 따른다: 사실마다 매체명·URL·일자를 병기하고, 불확실하면
  ⚠️, 확인 불가면 **(확인 필요)** 로 표기한다(§6). 질문당 요지 3~6문장의 간결한 답으로 정리하며,
  기업/산업 딥다이브급 주제면 별도 `/research` 실행을 제안하고 qa에는 요지+링크만 남긴다.
...
- 캡쳐 흐름: 사람은 `inbox/`(형식 자유)나 채팅에 질문 메모를 던진다. `/inbox` 스킬이 제목 확정 →
  frontmatter 부여 → 메모 보존 → 리서치 보강 → `raw/qa/` 저장 → [[raw-map]] 갱신 → `## 관련 노트`
  부착 → 원본 메모를 `inbox/old/`로 아카이브까지 대신한다.
