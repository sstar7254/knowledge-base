...
### qa — 질문-답변 노트 (사람의 생각이 원본인 raw)

qa는 **사용자의 질문과 그에 대한 자기 답(3~5문장)이 곧 원본**인 노트다. 외부 소스 요약이 아니라
사람의 생각을 불변 원본으로 보존한다. 아웃풋의 최소 단위이며, 마찰 없이 쓰는 것이 최우선이다.

- **파일명 = 질문 그대로.** 본문은 다듬지 않은 구어체 그대로 허용한다.
- **본문 없이 제목만 있어도 완결된 기록으로 유효하다**("이 시점에 이게 궁금했다"는 기록).
  비어 있는 본문을 LLM이 채워넣지 않는다 — 나중에 자연스럽게 떠오르면 사람이 채운다(망각 = 2차 필터).
- 출처 병기 의무 없음(선택).
- tags: 아래 공유 태그 리스트를 그대로 쓴다. 관심분야 매핑 — 공급망→`supply-chain`,
  지정학→`geopolitics/<대상>`, 스타트업→`vc`, 금융→`finance`.
- **예외 규칙**: qa 노트에 한해 LLM이 ① frontmatter ② 문서 말미 `## 관련 노트` 섹션의 추가·갱신까지
  허용한다(§2 첫머리의 "본문 수정 금지"의 유일한 예외). **질문·답 본문 자체는 여전히 불변이다.**
- 캡쳐 흐름: 사람은 `inbox/`(형식 자유)나 채팅에 제목+3~5문장만 던진다. `/inbox` 스킬이 frontmatter
  부여 → `raw/qa/` 저장 → [[raw-map]] 갱신 → `## 관련 노트` 부착을 대신한다.
