...
### 심화 질문    ← AI가 요약 기준으로 2~3개 생성. 각 질문 아래 답은 사람 (선택)
> [!note] AI 피드백   ← 요약·답변을 리서치와 대조한 누락·오해 1~2가지 (callout)
```
...
- **소화(output)는 선택이다.** `/inbox`·`/inbox-deep` 마무리 때 유도하거나 나중에 `/inbox-digest`로
  ...
  `## 소화`의 흐름은 ① 사람이 자기 언어로 요약 → ② AI가 심화 질문 2~3개 → ③ 답은 선택 →
  ④ AI가 리서치 대비 누락·오해를 피드백 callout으로 기록. 자세한 절차는 `/inbox-digest` 스킬이 규정한다.
