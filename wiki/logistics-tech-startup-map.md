...
> 인덱스 + 지형도 종합. 검색일 2026-06-22. 최근 3년 내 투자 또는 최근 뉴스가 조회되는 10개 기업을
> 6개 카테고리로 분류했다. 개별 분석은 각 노트, 출처 원본은 `raw/logistics-tech-startup/`.
...
| 확장 동력 | CAPEX 기반 센터 증설 | 채널 연동·데이터 | 소프트웨어로 분산자원 결합 |

## 3. 교차 분석 ① — 공통 페인포인트

1. **수작업·아날로그 의존**: 화물 중개/운영의 전화·이메일·스프레드시트, 창고의 수기 사이클카운트
   (Augment·FleetWorks·Dexory·테크타카).
2. **노동력 부족·인건비**: 트럭 운전자 부족(Waabi), 창고·라스트마일 인력(Dexory·UniUni).
3. **파편화·가시성 결여**: 유럽 운송 시장(sennder), 분산 창고·OMS/WMS/TMS(콜로세움), 멀티채널 셀러
   (테크타카).
4. **반품·과잉재고의 구조적 증가**: 이커머스 확대로 역물류 비효율·폐기 손실·ESG 부담(케이존).
5. **라스트마일 비용**: 물류비 최대 비중, 크로스보더 저가·대량 수요(UniUni).

## 4. 교차 분석 ② — 공통 PMF 전략 패턴

- **기존 워크플로우에 임베드(브라운필드 친화)**: 현장 프로세스·창고 구조를 바꾸지 않고 위에 얹는다
  (Dexory 로봇 투입, 음성 에이전트가 기존 통신 채널 사용 — FleetWorks).
...
- **AI 네이티브 SaaS의 빠른 후속**: Augment(시드 후 5개월 만 $85M), FleetWorks($17M) — 작은 팀이
  빠르게 트랙션을 증명.
- **한국**: 풀필먼트·역물류 중심으로 시리즈B~C가 활발(콜로세움 270억·테크타카 126억·케이존 63억),
  네이버 생태계가 주요 후원자. 콜로세움 2027 IPO 목표 등 **출구(IPO/SPAC)** 가시화 — UniUni SPAC
  상장 추진과 함께 후기 단계로 성숙 중.

## 6. 시사점

물류테크의 공통 분모는 **"파편화·수작업·노동력 부족"을 데이터·자동화·AI로 푸는 것**이다. 2026년의
