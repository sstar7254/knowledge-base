...
`raw/`의 기업분석 노트(`cov-[Ticker]-[Company].md`, `type: overview`) 전체 인덱스. **현행 frontmatter `tags`를 기준으로** 섹터를 구분한다. 각 기업은 **첫 번째(주) 태그** 섹션에 배치하고, 보조 태그는 끝에 `(+태그)`로 표기한다. 새 cov 노트를 추가하면 해당 태그 섹션에 링크를 추가한다.

> 섹터/산업 자체에 대한 리서치는 `raw/research-*.md`(예: [[research-semicon-value-chain]], [[research-Energy Industry]])에 있다.
...
- [[cov-AMD-Advanced Micro Devices]] : x86 CPU·AI 가속기 — 서버·PC용 EPYC/Ryzen CPU와 Instinct 가속기를 파는 팹리스로 엔비디아의 유일한 범용 GPU 대안. (+semiconductor)
...
- [[cov-JOBY-Joby Aviation]] : UAM/eVTOL — 전기 수직 이착륙기(eVTOL) 개발 및 도심 항공 모빌리티 서비스 상용화 추진. (+aerospace)
