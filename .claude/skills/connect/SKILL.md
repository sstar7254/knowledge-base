...
- 1차: `memory.md`의 `last_commit` 이후 변경분을 git으로 추린다.
  `git diff --name-only <last_commit> HEAD -- 'wiki/**/*.md'` + 추적 안 된 신규 파일(`git status`).
- 2차(보강): `wiki/`의 전체 노트 집합에서 memory.md `## 처리됨` 목록에 없는 파일을 신규로 본다.
...
2. 가장 밀접한 **기존 노트 3~6개**를 고른다. 좋은 연결의 근거는 예를 들어:
...
3. 노트 끝에 아래 형식으로 섹션을 **추가(append)** 한다(있으면 합치고, 헤딩을 중복 생성하지 않는다):
   ...
   한 줄에 관계 유형별로 묶고, 짧은 한국어 설명을 앞에 붙인다.
