...
1. **운영 규칙 로드** — `schema/AGENT.md`를 먼저 읽어 3계층 구조·명명·쓰기 규칙을 확인한다.
...
- **모든 핵심 진술은 출처 노트로 위키링크**한다. 사실의 근거는 `raw/`로, 사례 기업은 `raw/cov/`의 overview 노트로,
  개념은 `wiki/geopolitics/` 등으로 잇는다(`schema/AGENT.md` §3.4).
...
1. **wiki-map** — `wiki/wiki-map.md`의 `cross-border` 섹션에 새 노트를 **직접** 한 줄 링크로
   추가하고 `updated`를 갱신한다.
2. **파일트리** — `wiki/` 구조가 바뀌면 `schema/AGENT.md`와 `README.md`의
   파일 트리에 모두 반영한다.
...
- 기본 브랜치에 직접 커밋하지 않는다(`schema/AGENT.md` §0). 작업 브랜치에서 진행한다.
